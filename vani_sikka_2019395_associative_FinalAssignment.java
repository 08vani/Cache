...
public class vani_sikka_2019395_associative_FinalAssignment
{
	...
	static LinkedList<String> LRU=new LinkedList<>();
	static Map<String , Integer> CA_Lines=  new HashMap<>();
	...
	private static void write_cache(String adrs, String data,int n,int p,int w,int memory_size,int block_size,int cache_size,int total_blocks,cache_line[] cache) 
	{
		...
		//System.out.println(word_no+"*"+block_no);
		...
		for(int i=0;i<cache.length;i++)
		{
			...
			if(cache[i].tag.equals(block_no)==true)
			{
				...
				CA_Lines.put(block_no,i);
				LRU.addFirst(block_no);
				break;
			}
		...
		if(is_space==true && is_hit==false)
		{
			...
			CA_Lines.put(block_no,replace_line);
			LRU.addFirst(block_no);
			System.out.println("Block to be inserted: "+block_no);
		}
		if(is_space==false && is_hit==false)
		{
			String rv=LRU.removeLast();
			int cl=CA_Lines.get(rv);
			CA_Lines.remove(rv);
			System.out.println("MISS");
			memory.put(adrs,data);
			insert_block(word_no,block_no,cache,cl,block_size,n,w,cache_size);
			System.out.println("Block to be inserted: "+block_no);
			CA_Lines.put(block_no,cl);
			LRU.addFirst(block_no);
		}
