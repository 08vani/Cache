...
public class vani_sikka_2019395_set_associative_FinalAssignment
{
	...
	public static void main(String[] args) 
	{
		...
		String[] i=inc.split(":");
		int memory_size=Integer.valueOf(i[0]);//in bytes(word size)i.e memory_size addresses in memory of one word each hence is the number of addresses in memory
		...
		while(str.equals("STOP")==false)
		{
			...
			if(arr.length==1)
			{
				...
				read_cache(arr[0],n,p,w,memory_size,block_size,cache_size,total_blocks,cache,v,d);
				print_cache(cache);
			}
			else 
			{
				//System.out.println(arr[0]+" "+arr[1]);
				write_cache(arr[0],arr[1],n,p,w,memory_size,block_size,cache_size,total_blocks,cache,v,d);
				print_cache(cache);
			}
			str=in.nextLine();//taking the address as inputs in case of loading and taking address and data as input in case of writing
		}
	...
	private static void write_cache(String adrs, String data,int n,int p,int w,int memory_size,int block_size,int cache_size,int total_blocks,cache_line[] cache,int v,int d) 
		{
			...
			String tag=block_no.substring(0,((n-w-d)-1));//tag is the block no to which the address belong
			int no_sets=cache.length/v;
			int set_no=binary_to_decimal(block_no) % no_sets;//it is the expected index where the tag should be there as cache_line assigned to a tag is with formula block_size mod cache_size.Since this formula can map many block to a single cache_line hence we have to check that whether the tag or block address in the input memory address is same as the tag past in the cache line
			int sn=set_no*v;
			boolean is_hit=false,is_space=false;
			int replace_line=0;
			//System.out.println(sn+"##"+set_no+"##");
			for(int i=sn,j=0;j<v;i++,j++)
			{
				if(cache[i].tag.equals("-")==true)
				{
					is_space=true;
					replace_line=i;
				}
				if(cache[i].tag.equals(block_no)==true)
				{
					is_hit=true;
					System.out.println("HIT");
					int word_index=binary_to_decimal(word_no);
					cache[i].block[word_index]=data;
					memory.put(adrs,data);
					break;
				}
				
			}
			//System.out.println(replace_line+"@@");
			if(is_space==true && is_hit==false)
			{
				System.out.println("MISS");
				memory.put(adrs,data);
				insert_block(word_no,block_no,cache,replace_line,block_size,n,w,cache_size);
				System.out.println("Block to be inserted: "+block_no);
			}
			if(is_space==false && is_hit==false)
			{

				System.out.println("MISS");
				memory.put(adrs,data);
				insert_block(word_no,block_no,cache,sn,block_size,n,w,cache_size);
				System.out.println("Block to be inserted: "+block_no);
			}
//			for(int j=0;j<cache.length;j++)
//			{
//				System.out.println(cache[j].tag);
//			}
//			System.out.println("**********************");
//			
			
		}
	public static void read_cache(String adrs,int n,int p,int w,int memory_size,int block_size,int cache_size,int total_blocks,cache_line[] cache,int v,int d)
		{
			...
			String tag=block_no.substring(0,((n-w-d)-1));//tag is the block no to which the address belong
			int no_sets=cache.length/v;
			int set_no=binary_to_decimal(block_no) % no_sets;
			int sn=set_no*v;
			boolean is_hit=false,is_space=false;
			int replace_line=0;
			//System.out.println(word_no+"*"+block_no+"*"+sn);
			for(int i=sn,j=0;j<v;i++,j++)
			{
				if(cache[i].tag.equals(block_no)==true)
				{
					...
					int word_index=binary_to_decimal(word_no);
					//System.out.println(cache[i].block[word_index]);
			...
			if(is_space==true && is_hit==false)
			{
				System.out.println("MISS");
				System.out.println("Data read:");
				read_main_memory(adrs,word_no,block_no,cache,replace_line,block_size,n,w,cache_size);
				System.out.println("Block to be inserted: "+block_no);
			}
			if(is_space==false && is_hit==false)
			{
...
//			System.out.println("**********************");
	...
	private static void read_main_memory(String adrs,String wd_no,String bl_no,cache_line[] cache,int cl,int block_size,int n,int w,int cache_size) 
		{
			...
			insert_block(wd_no,bl_no,cache,cl,block_size,n,w,cache_size);
			
				
			
		}
	private static void insert_block(String wd_no,String bl_no,cache_line[] cache,int cl,int block_size,int n,int w,int cache_size) 
		{
